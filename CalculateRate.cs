...
{
    private static readonly CompensationDatabaseService DbService = new  ();
    ...
    {
        return await DbService.GetRate(disabilityPercentage, married, parents, children);
    }
    ...
    public static async Task<float> CalculateTotalCompensation(Veteran veteran)
    {
        ...
        if (veteran.DisabilityPercentage >= 30)
        {
            ...
            {
                float childBonusUnder18 = await DbService.GetAdditionalRate(veteran.DisabilityPercentage, "ChildUnder18");
            ...
            {
                float childBonusOver18InSchool = await DbService.GetAdditionalRate(veteran.DisabilityPercentage, "ChildOver18School");
                rate += veteran.ChildrenOver18InSchoolCount * childBonusOver18InSchool;
            }
        
            // Add for spouse Aid and Attendance
            if (veteran is {IsMarried: true, SpouseReceivingAidAndAttendance: true, DisabilityPercentage: >= 70})
            {
                float aidAndAttendanceBonus = await DbService.GetAdditionalRate(
